...
-------------------------------------------------------------
The GUI shells out to the same `roastnet` CLI a terminal user would run,
...
def roastnet_argv(*args: str) -> list[str]:
    ...
    Using sys.executable rather than a bare "roastnet" matters: this project
    is normally used from a virtualenv (.venv/bin/roastnet), and a bare name
    would find the system Python's copy -- or nothing at all.

    Under a PyInstaller-frozen roastnet-gui, sys.executable IS the frozen
    bootloader, not a Python interpreter that understands "-m roastnet.cli"
    -- so instead shell out to the sibling `roastnet` binary, which the
    packaged distribution always ships alongside it (see packaging/).
    """
    if getattr(sys, "frozen", False):
        exe_dir = Path(sys.executable).resolve().parent
        cli_name = "roastnet.exe" if sys.platform == "win32" else "roastnet"
        return [str(exe_dir / cli_name), *args]
    return [sys.executable, "-m", "roastnet.cli", *args]
