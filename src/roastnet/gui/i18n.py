...
text, the chart readout. The `roastnet` CLI's own output (which streams
...
`src/roastnet/gui/locales/<code>.json`, read the same way this project
...
The language is chosen once, at startup (RoastnetApp.__init__, before any
...
def _load_catalog(code: str) -> dict[str, str]:
    ...
    try:
        raw = resources.files("roastnet.gui").joinpath(f"locales/{code}.json").read_text(encoding="utf-8")
        data = json.loads(raw)
...
def resolve_language(configured: str | None) -> str:
    """Precedence: $ROASTNET_LANG env override (matches the existing
    $ROASTNET_UI_SCALE/$ROASTNET_LINE_SCALE convention in gui/widgets.py)
    ...
    env = os.environ.get("ROASTNET_LANG", "").strip().lower()
    if env in LANGUAGES:
        return env
